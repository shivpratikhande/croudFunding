...
import { concatUint8Arrays, uint8ArrayToBase64, } from "../../../../../utils/uint8-array.js";
...
import { AWS_REGION, ENCLAVE_KMS_KEY_ARN, ROUTE_AUTH_COGNITO_ID_TOKEN_V2, } from "../constants.js";
...
export async function migrateToEnclaveWallet(args) {
    ...
    const key = await QuickCrypto.subtle.importKey("raw", plaintextKeyBlob, "AES-CBC", false, ["encrypt", "decrypt"]);
    // @ts-ignore - default import buils but ts doesn't like it
...
async function executeMigration(args) {
    ...
    if (!migrationResponse.ok) {
        throw new Error(`Failed to migrate to enclave wallet: ${JSON.stringify(await migrationResponse.json())}`);
    }
